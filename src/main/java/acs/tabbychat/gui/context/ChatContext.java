...
public abstract class ChatContext extends GuiButton {
	...
	public void drawButton(Minecraft mc, int x, int y){
		...
		if(getDisplayIcon() != null)
			drawIcon();
		this.drawString(mc.fontRenderer, this.displayString, xPosition + 18, yPosition + 4, getStringColor());
		if(this.getChildren() != null){
			// This has children.
			int length = mc.fontRenderer.getCharWidth('>');
			this.drawString(mc.fontRenderer, ">", xPosition + width - length, yPosition + 4, getStringColor());
	...
	protected void drawBorders(){
		...
		Gui.drawRect(xPosition + height, yPosition, xPosition + height + 1, yPosition + height, -0xffffff);
	}
	...
	/**
	 * How this item displays when {@code isPositionValid(int, int)} return false.
	 */
